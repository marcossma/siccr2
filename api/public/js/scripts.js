...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // ═══════════════════════════════════════════════════════════════
    // ENSALAMENTO EM LOTE — /ensalamento (direção; menu Administrativo)
    // ═══════════════════════════════════════════════════════════════
    if (urlParam === "/ensalamento") {
        const siccrE = JSON.parse(localStorage.getItem("siccr") || "null");
        const ehDirecao = siccrE && (["super_admin", "diretor", "vice_diretor"].includes(siccrE.permissao) || siccrE.is_direcao_centro === true);
        if (!ehDirecao) { window.location.href = "/"; return; }

        const $ = (id) => document.getElementById(id);
        const NOMES_DIAS = ["Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"];
        const selPeriodo = $("ensPeriodo"), selCurso = $("ensCurso"), selDia = $("ensDia");
        const chkPos = $("ensIncluirPos"), chkCap = $("ensRespeitarCap");
        const btnCarregar = $("ensCarregar"), btnAuto = $("ensAuto"), btnAplicar = $("ensAplicar");
        const tbody = $("ensLista"), resumo = $("ensResumo");

        let fila = [];               // horários órfãos carregados
        const horaCurta = (v) => String(v || "").slice(0, 5);

        async function carregarBases() {
            const [rp] = await Promise.all([fetch(`${apiUrl}/periodos-letivos`).then(r => r.json())]);
        ...
        async function carregarCursosFiltro() {
            ...
            selCurso.value = cursos.some(c => String(c.id_curso) === anterior) ? anterior : "";
        }

        function badgeFit(cabe) {
            if (cabe === true) return '<span class="badge-fit fit-ok">cabe</span>';
            if (cabe === false) return '<span class="badge-fit fit-no">pequena</span>';
            return '<span class="badge-fit fit-na">—</span>';
        }

        // Recalcula o "cabe" de uma linha comparando a capacidade da sala escolhida com as vagas
        function recalcFit(tr) {
            const sel = tr.querySelector(".ens-sala-sel");
            const opt = sel.options[sel.selectedIndex];
            const vagas = tr.dataset.vagas ? parseInt(tr.dataset.vagas, 10) : null;
            const cap = opt && opt.dataset.cap ? parseInt(opt.dataset.cap, 10) : null;
            const cabe = (vagas !== null && cap !== null) ? cap >= vagas : null;
            tr.querySelector(".ens-fit").innerHTML = sel.value ? badgeFit(cabe) : "";
        }

        function atualizarResumo() {
            const linhas = tbody.querySelectorAll("tr[data-horario]");
            const escolhidas = [...linhas].filter(tr => tr.querySelector(".ens-sala-sel")?.value).length;
            resumo.textContent = `${linhas.length} horário(s) na fila · ${escolhidas} com sala escolhida`;
            btnAplicar.disabled = escolhidas === 0;
        }

        function paramSlot(tr) {
            const p = new URLSearchParams({
                dia_semana: tr.dataset.dia,
                hora_inicio: tr.dataset.hi,
                hora_fim: tr.dataset.hf,
            });
            if (tr.dataset.blocoInicio && tr.dataset.blocoFim) {
                p.set("data_inicio", tr.dataset.blocoInicio);
                p.set("data_fim", tr.dataset.blocoFim);
            } else if (tr.dataset.periodo) {
                p.set("periodo_letivo_id", tr.dataset.periodo);
            }
            if (tr.dataset.vagas) p.set("vagas", tr.dataset.vagas);
            return p.toString();
        }

        // Carrega (uma vez) as salas disponíveis daquela linha, preservando a seleção atual
        async function carregarSalasLinha(tr) {
            if (tr.dataset.loaded === "1") return;
            const sel = tr.querySelector(".ens-sala-sel");
            const selecionada = sel.value;
            sel.disabled = true;
            try {
                const salas = ((await (await fetch(`${apiUrl}/salas/disponiveis?${paramSlot(tr)}`)).json()).data) || [];
                const opcoes = salas.map(s => {
                    const marca = s.cabe === true ? " ✓" : s.cabe === false ? " ⚠" : "";
                    return `<option value="${s.sala_id}" data-cap="${s.sala_capacidade ?? ""}">${s.sala_nome}${s.sala_capacidade !== null ? ` (${s.sala_capacidade})` : ""}${marca}</option>`;
                }).join("");
                sel.innerHTML = '<option value="">— nenhuma —</option>' + opcoes;
                // Mantém a sala sugerida mesmo que já não conste na lista (ex.: veio do auto)
                if (selecionada && !salas.some(s => String(s.sala_id) === selecionada)) {
                    const nome = tr.dataset.salaNome || `Sala ${selecionada}`;
                    sel.insertAdjacentHTML("beforeend", `<option value="${selecionada}" data-cap="${tr.dataset.salaCap || ""}">${nome} (sugerida)</option>`);
                }
                sel.value = selecionada;
                tr.dataset.loaded = "1";
            } catch { sel.innerHTML = '<option value="">erro ao carregar</option>'; }
            finally { sel.disabled = false; recalcFit(tr); }
        }

        function linhaHTML(h) {
            const bloco = h.bloco_inicio ? String(h.bloco_inicio).slice(0, 10) : "";
            const blocoFim = h.bloco_fim ? String(h.bloco_fim).slice(0, 10) : "";
            return `<tr data-horario="${h.id_horario}" data-dia="${h.dia_semana}" data-hi="${horaCurta(h.hora_inicio)}" data-hf="${horaCurta(h.hora_fim)}"
                        data-vagas="${h.vagas ?? ""}" data-periodo="${h.periodo_letivo_id ?? ""}" data-bloco-inicio="${bloco}" data-bloco-fim="${blocoFim}">
                <td>${h.disciplina_codigo ? h.disciplina_codigo + " - " : ""}${h.disciplina_nome || ""}</td>
                <td>${h.nome_turma || ""}</td>
                <td>${h.curso_nome || "—"}</td>
                <td class="ens-dia">${NOMES_DIAS[h.dia_semana] || "?"}</td>
                <td class="ens-hora">${horaCurta(h.hora_inicio)}–${horaCurta(h.hora_fim)}</td>
                <td class="ens-vagas">${h.vagas ?? "—"}</td>
                <td><select class="formInput ens-sala-sel"><option value="">— escolher —</option></select></td>
                <td class="ens-fit"></td>
                <td class="ens-status"></td>
            </tr>`;
        }

        async function carregarFila() {
            const p = new URLSearchParams();
            if (selPeriodo.value) p.set("periodo_letivo_id", selPeriodo.value);
            if (selCurso.value) p.set("curso_id", selCurso.value);
            if (selDia.value !== "") p.set("dia_semana", selDia.value);
            if (chkPos.checked) p.set("incluir_pos", "1");
            btnCarregar.disabled = true; btnCarregar.textContent = "Carregando…";
            try {
                fila = ((await (await fetch(`${apiUrl}/turmas/ensalamento?${p.toString()}`)).json()).data) || [];
                if (fila.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="9" class="ens-vazio">Nenhum horário sem sala para esses filtros. 🎉</td></tr>';
                } else {
                    tbody.innerHTML = fila.map(linhaHTML).join("");
                }
                atualizarResumo();
            } finally { btnCarregar.disabled = false; btnCarregar.textContent = "Carregar fila"; }
        }

        async function sugerirAuto() {
            if (fila.length === 0) { alert("Carregue a fila primeiro."); return; }
            btnAuto.disabled = true; btnAuto.textContent = "Sugerindo…";
            try {
                const body = {
                    respeitar_capacidade: chkCap.checked,
                    incluir_pos: chkPos.checked,
                };
                if (selPeriodo.value) body.periodo_letivo_id = parseInt(selPeriodo.value, 10);
                if (selCurso.value) body.curso_id = parseInt(selCurso.value, 10);
                if (selDia.value !== "") body.dia_semana = parseInt(selDia.value, 10);
                const resp = await (await fetch(`${apiUrl}/turmas/ensalamento/auto`, {
                    method: "POST", body: JSON.stringify(body),
                })).json();
                const mapa = new Map((resp.data?.proposta || []).map(p => [String(p.horario_id), p]));
                let sugeridas = 0;
                tbody.querySelectorAll("tr[data-horario]").forEach(tr => {
                    const prop = mapa.get(tr.dataset.horario);
                    const sel = tr.querySelector(".ens-sala-sel");
                    const stat = tr.querySelector(".ens-status");
                    if (prop && prop.sala_id) {
                        // injeta a sala sugerida como opção selecionada (lista completa carrega ao focar)
                        tr.dataset.salaNome = prop.sala_nome || "";
                        tr.dataset.salaCap = prop.sala_capacidade ?? "";
                        tr.dataset.loaded = "";
                        sel.innerHTML = `<option value="">— escolher —</option>` +
                            `<option value="${prop.sala_id}" data-cap="${prop.sala_capacidade ?? ""}" selected>${prop.sala_nome}${prop.sala_capacidade !== null ? ` (${prop.sala_capacidade})` : ""} (sugerida)</option>`;
                        recalcFit(tr);
                        stat.innerHTML = "";
                        sugeridas++;
                    } else if (prop) {
                        stat.innerHTML = `<span class="ens-msg">${prop.motivo || "sem sugestão"}</span>`;
                    }
                });
                resumo.textContent = `Sugestão automática: ${sugeridas} de ${fila.length} horário(s) receberam sala.`;
                atualizarResumo();
            } finally { btnAuto.disabled = false; btnAuto.textContent = "Sugerir automaticamente"; }
        }

        async function aplicar() {
            const itens = [];
            tbody.querySelectorAll("tr[data-horario]").forEach(tr => {
                const salaId = tr.querySelector(".ens-sala-sel")?.value;
                if (salaId) itens.push({ horario_id: parseInt(tr.dataset.horario, 10), sala_id: parseInt(salaId, 10) });
            });
            if (itens.length === 0) { alert("Nenhuma sala escolhida."); return; }
            if (!confirm(`Aplicar ensalamento de ${itens.length} horário(s)?`)) return;
            btnAplicar.disabled = true; btnAplicar.textContent = "Aplicando…";
            try {
                const resp = await (await fetch(`${apiUrl}/turmas/ensalamento/lote`, {
                    method: "POST", body: JSON.stringify({ itens }),
                })).json();
                const mapa = new Map((resp.data?.resultados || []).map(r => [String(r.horario_id), r]));
                tbody.querySelectorAll("tr[data-horario]").forEach(tr => {
                    const r = mapa.get(tr.dataset.horario);
                    if (!r) return;
                    const stat = tr.querySelector(".ens-status");
                    if (r.ok) {
                        tr.classList.add("ok"); tr.classList.remove("falha");
                        stat.innerHTML = `<span style="color:#2b8a3e;font-weight:600">✓ ${r.total} aulas</span>`;
                        tr.querySelector(".ens-sala-sel").disabled = true;
                        tr.removeAttribute("data-horario"); // sai da fila de pendentes
                    } else {
                        tr.classList.add("falha"); tr.classList.remove("ok");
                        stat.innerHTML = `<span class="ens-msg">${r.message || "falhou"}</span>`;
                    }
                });
                resumo.textContent = resp.message || "";
                atualizarResumo();
            } finally { btnAplicar.disabled = false; btnAplicar.textContent = "Aplicar ensalamento"; }
        }

        // Eventos
        tbody.addEventListener("focusin", (e) => {
            const tr = e.target.closest("tr[data-horario]");
            if (tr && e.target.classList.contains("ens-sala-sel")) carregarSalasLinha(tr);
        });
        tbody.addEventListener("change", (e) => {
            const tr = e.target.closest("tr");
            if (tr && e.target.classList.contains("ens-sala-sel")) { recalcFit(tr); atualizarResumo(); }
        });
        chkPos.addEventListener("change", carregarCursosFiltro);
        btnCarregar.addEventListener("click", carregarFila);
        btnAuto.addEventListener("click", sugerirAuto);
