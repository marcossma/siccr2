...
router.get("/total-info", async(req, res) => {
    try {
        ...
        console.error("Erro ao tentar listar subunidades (total-info): ", error);
        res.status(500).json({
            status: "error",
            message: "Erro ao tentar listar todas as informações das subunidades.",
            data: ""
        });
    }
});
...
router.put("/:idsubunidade", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            status: "error",
            message: "Erro ao tentar atualizar subunidade.",
            data: ""
        });
    }
});
